...
function _generateAccountId({ full_name }) {
  const initials = full_name.slice(0, 2).toUpperCase();
...
async function _sendEmail({ email, accountID, businessOwnerID }) {
  const mail_body = BusinessAccount(
    accountID,
    "imgs/logo_ios.png",
  ...
  sendMail(email, mail_body)
    .then(() => {
      ...
      AccountID.create(
        { account_id: accountID, business_owner: businessOwnerID },
    ...
    .catch((err) => {
      throw Error(`failed to send mail ${err}`);
    });
}
...
async function createBusinessOwner({
  ...
  try {
    ...
    if (result) {
      ...
      await _sendEmail({ email, accountID, businessOwnerID: result._id });
    }
    return { status: 200, message: "Owner created successfully", data: result };
  } catch (err) {
    return { status: 400, message: err.msg };
  }
}
...
async function _getLoggedInUserType(result, password) {
  if (!result) {
    return { status: "failed", message: "please provide correct credentials" };
  }
