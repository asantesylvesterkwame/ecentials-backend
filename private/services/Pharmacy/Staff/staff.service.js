...
async function createPharmacyStaff({ req }) {
    try {
        ...
        const photo_url = await uploadFile(req.files['photo'][0] ,`pharmacyStaff/${employee_id}`)
        const cv_url = await uploadFile(req.files['cv'][0] ,`pharmacyStaff/${employee_id}`)
        const certificate_url = await uploadFile(req.files['certificate'][0] ,`pharmacyStaff/${employee_id}`)
