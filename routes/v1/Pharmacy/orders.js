...
router.post('/fetch-all-orders', verify, async (req, res) => {
    ...
    try{
        const orders = await Orders.find({store_id}, {_id:1, order_code:1, payment_type:1, payment_status:1, grand_total:1, order_status:1, createdAt: 1})
...
router.post('/fetch-specific-orders', verify, async (req, res) => {
    ...
    try{
        ...
        orders.forEach(async element => {
            ...
            order_details.customer_name = await fetchUsersName(user_id);/*extracting users name from personal details*/
            ...
            if(orders) return res.status(200).json({ message: "success", data: order_details })
